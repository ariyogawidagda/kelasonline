...
<div class="col-lg-9">
<form method="post" action="/admin/inputkelas/{{ $kelas->id }}">
        @csrf
        <div class="mb-3">
            <label for="name" class="form-label">Nama Mata Pelajaran</label>
            <input type="text" value="{{ $kelas->nama_mapel }}" class="form-control @error('name') is-invalid @enderror" id="name" name="namemapel">
            @error('name')
        ...
        <button type="submit" class="mt-2 btn btn-primary">Edit Data</button>
    </form>
</div>
@endsection